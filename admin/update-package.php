...
{
    // Get package image name to delete the file
    $sql_image = "SELECT PackageImage FROM TblTourPackages WHERE PackageId=:pid";
    $query_image = $dbh->prepare($sql_image);
    $query_image->bindParam(':pid', $pid, PDO::PARAM_STR);
    $query_image->execute();
    $result_image = $query_image->fetch(PDO::FETCH_ASSOC);
    
    // Delete the package from database
    $sql_delete = "DELETE FROM TblTourPackages WHERE PackageId=:pid";
    $query_delete = $dbh->prepare($sql_delete);
    $query_delete->bindParam(':pid', $pid, PDO::PARAM_STR);
    $query_delete->execute();
    
    if($query_delete->rowCount() > 0) {
        // Delete the image file if it exists
        if($result_image && file_exists("pacakgeimages/".$result_image['PackageImage'])) {
            unlink("pacakgeimages/".$result_image['PackageImage']);
        }
...
<html>
<head>
		<title>Prayanam | Admin Package Creation</title>
...
<?php echo htmlentities($result->UpdationDate);?>
			...
			<div class="col-sm-8 col-sm-offset-2">
				...
				<button type="submit" name="delete" class="btn-danger btn" onclick="return confirm('Are you sure you want to delete this package? This action cannot be undone.')" style="margin-left: 10px;">Delete Package</button>
				<a href="manage-packages.php" class="btn btn-default" style="margin-left: 10px;">Back to Packages</a>
			</div>
		</div>
